...
-- Requires Neovim 0.10+ (uses vim.uv and vim.api.nvim_set_option_value).
-- Lazy-native usage:
...
local cfg = {
  ...
  dry_run = false,
  -- Close the renamer buffer after a successful apply.
...
local function dir_is_empty(p)
  ...
  return uv.fs_scandir_next(h) == nil
end
...
local function list_files(patterns)
  ...
  -- ripgrep returns 1 for "no matches" in some modes; treat 0/1 as ok
  if code ~= 0 and code ~= 1 then
    vim.notify("renamer: ripgrep command failed (exit " .. tostring(code) .. "). Check rg_base/patterns.",
...
-- robust move with libuv fallback (and cross-device copy+unlink)
local function move_path(src, dst)
  -- try plain rename first (fast path)
  local ok = os.rename(src, dst)
  if ok then return true end

  -- libuv rename, then cross-device fallback
...
local function open_buf_with(lines, opts)
  ...
  vim.api.nvim_create_autocmd("BufWriteCmd", {
    ...
    callback = function(args)
      ...
      if dry_run then
        local preview = {}
        for _, m in ipairs(moves) do table.insert(preview, m.from .. " -> " .. m.to) end
        vim.cmd("new")
        vim.api.nvim_buf_set_lines(0, 0, -1, false, preview)
        vim.notify(("renamer: dry run (%d change%s)")
          :format(#moves, #moves == 1 and "" or "s"))
      ...
      -- Phase A: for any move where `to` is also a source (and != from), move `from` -> temp
      ...
      -- Prune empty directories left behind (sources' parents)
      ...
      if removed > 0 then
        vim.notify(("renamer: applied %d rename%s; removed %d empty dir%s")
          :format(#moves, #moves == 1 and "" or "s", removed, removed == 1 and "" or "s"))
      else
        vim.notify(("renamer: applied %d rename%s")
          :format(#moves, #moves == 1 and "" or "s"))
      end

      -- Close the renamer buffer after success (defer until after autocmd returns)
      if cfg.close_on_success then
